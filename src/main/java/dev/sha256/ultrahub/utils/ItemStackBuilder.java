package dev.sha256.ultrahub.utils;

import dev.sha256.ultrahub.Ultrahub;
import dev.sha256.ultrahub.hook.hooks.head.HeadHook;
...
public class ItemStackBuilder {
    ...
    private static final Ultrahub PLUGIN = JavaPlugin.getPlugin(Ultrahub.class);
    ...
    public static ItemStackBuilder getItemStack(ConfigurationSection section, Player player) {
        ...
        if (item.getType() == XMaterial.PLAYER_HEAD.parseMaterial()) {
            if (section.contains("base64")) {
                item = ((HeadHook) PLUGIN.getHooksManager().getPluginHook("BASE64")).getHead(section.getString("base64")).clone();
            } else if (section.contains("hdb") && PLUGIN.getHooksManager().isHookEnabled("HEAD_DATABASE")) {
                item = ((HeadHook) PLUGIN.getHooksManager().getPluginHook("HEAD_DATABASE")).getHead(section.getString("hdb"));
            }
        }
